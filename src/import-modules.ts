...
async function asyncHandler(options: ImportedModulesPrivateOptions): Promise<ImportedModules> {
  ...
  for (const filePath of filesPaths) {
    const isModuleImported = await importModule(filePath, index, options, modules, true);
 ...
 * @param {boolean} isAsync - Determines if the to be loaded module is async or not.
...
function importModule(
  ...
  if (isAsync)
    return new Promise<boolean>((resolve) => {
      resolve(
        import(filePath).then((importedModule) => {
